...
def make_openmm_script(pdb_filename: str, ff_files: list[str],
                        water_files: list[str], box: dict) -> str:
    ff_list = ", ".join(f"'{f}'" for f in ff_files + water_files)
...
# Force fields : {', '.join(ff_files + water_files)}
...
def _collect_ff_files(ff_choices: dict) -> tuple[list, list, str]:
    """Return (ff_files, water_files, ecosystem)."""
    ff_files, eco = [], None
    if ff_choices.get("protein") in FF_PROTEIN:
...
def generate_files_job(prepared_path: Path, ff_choices: dict, box: dict,
    ...
    job_running = True
    try:
        ff_files, water_files, eco = _collect_ff_files(ff_choices)
        ...
        if engine == "openmm":
            ...
            readme = (
                ...
                f"FF        : {', '.join(ff_files + water_files)}\n\n"
                f"Run on server:\n"
            ...
            ff      = mm.ForceField(*(ff_files + water_files))
